...
export function weeklyDays(selection: WeeklyDaySelection): number[] {
  return selection.mode === 'days'
    ? [...new Set(selection.days)].sort((a, b) => a - b)
    : Array.from(
        { length: ((selection.end - selection.start + 7) % 7) + 1 },
        (_, i) => ((selection.start - 1 + i) % 7) + 1,
      );
}

/** Boundary validation shared by desktop IPC, runtime commands and model tools. */
export function parseWeeklyRule(value: unknown): TaskRuleWeekly | undefined {
  ...
  if (
    ...
    return undefined;
  if (typeof rule.startDate !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(rule.startDate))
    return undefined;
  const date = new Date(`${rule.startDate}T00:00:00Z`);
  if (
    !Number.isFinite(date.getTime()) ||
    date.getUTCFullYear() < 1970 ||
    date.toISOString().slice(0, 10) !== rule.startDate
  )
    return undefined;
  if (!rule.selection || typeof rule.selection !== 'object') return undefined;
  const selection = rule.selection as Record<string, unknown>;
  const isDay = (day: unknown): day is number =>
    typeof day === 'number' && Number.isInteger(day) && day >= 1 && day <= 7;
  let parsed: WeeklyDaySelection;
  if (
    selection.mode === 'days' &&
    Array.isArray(selection.days) &&
    selection.days.length > 0 &&
    selection.days.length <= 7 &&
    selection.days.every(isDay)
  ) {
    parsed = { mode: 'days', days: [...new Set(selection.days as number[])].sort((a, b) => a - b) };
  } else if (selection.mode === 'range' && isDay(selection.start) && isDay(selection.end)) {
    parsed = { mode: 'range', start: selection.start, end: selection.end };
  } else return undefined;
  return { kind: 'weekly', selection: parsed, time: rule.time, startDate: rule.startDate };
}

export function weeklyRuleSummary(rule: TaskRuleWeekly): string {
  const selection = rule.selection;
  const days =
    selection.mode === 'days'
      ? weeklyDays(selection)
          .map((day) => WEEKDAY_LABELS[day - 1])
          .join('、')
      : selection.start === selection.end
        ? WEEKDAY_LABELS[selection.start - 1]
        : `${WEEKDAY_LABELS[selection.start - 1]}至${WEEKDAY_LABELS[selection.end - 1]}`;
  return `每${days}，${rule.time} 开始`;
}
...
export function nextWeeklyRunAt(
  ...
  // nextRun is exclusive of startAt: subtract one second so effective-date
  // midnight is included. CronDate handles task-local dates and timezone offsets.
  const midnight = new CronDate(`${rule.startDate}T00:00:00`, timeZone).getDate();
  const cron = new Cron(
    `${minute} ${hour} * * ${weeklyDays(rule.selection)
      .map((day) => day % 7)
      .join(',')}`,
    {
      timezone: timeZone,
      startAt: new Date(midnight.getTime() - 1000),
    },
  );
  return cron.nextRun(after)?.toISOString();
}
...
function localMidnightUtc(date: string, timeZone: string): number {
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    guess = Date.parse(
      `${new Date(guess + delta * 24 * 60 * 60_000).toISOString().slice(0, 10)}T12:00:00Z`,
    );
  }
...
export function computeNextRunAt(
  ...
  switch (rule.kind) {
    case 'weekly':
      return nextWeeklyRunAt(rule, task.timeZone, new Date(firedAt));
    case 'at':
      return undefined;
    case 'every': {
...
/** 首次触发时间推算（创建任务时）。 */
export function initialNextRunAt(task: ScheduledTask, now: Date = new Date()): string | undefined {
  const rule = task.rule;
  if (rule.kind === 'weekly') return nextWeeklyRunAt(rule, task.timeZone, now);
