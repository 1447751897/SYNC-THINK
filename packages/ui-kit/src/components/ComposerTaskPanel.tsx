...
export interface ComposerTaskProjection {
...
export interface ComposerTaskPanelProps {
  todo?: ComposerTaskProjection | null;
  scopeKey?: string;
}

function taskSignature(todo: ComposerTaskProjection): string {
  return JSON.stringify(
    todo.items.map((item) => [item.status, item.title, item.description ?? '']),
  );
}

export function ComposerTaskPanel({ todo, scopeKey = '__default__' }: ComposerTaskPanelProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div
            className="shell-composer-task-panel__list"
            data-testid="composer-task-list"
