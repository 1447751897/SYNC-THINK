...
function modeBannerTransitionMs(kind: ComposerModeKind): number {
  return motionDurationMs(
    kind === 'plan' ? NEWMAX_PLAN_BANNER_TRANSITION_MS : NEWMAX_GOAL_BANNER_TRANSITION_MS,
  );
}
...
export function NewMaxComposerFrame({
  ...
  const currentModeKey = hasModeBanner ? resolveModeBannerKey(modeBanner, currentModeKind) : 'none';
