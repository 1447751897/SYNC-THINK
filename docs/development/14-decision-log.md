...
### DEC-20260716-007: 项目独立于本地文件夹，任务归属项目
...
```text
现有界面把 Workspace、项目、文件夹和任务混在同一层：用户必须先选择目录，目录被当作顶层对象，任务再挂在目录下。目录选择后还依赖 window.prompt 命名，导致创建请求可能静默中止。
```

最终选择：

```text
项目是独立顶层对象，可先创建且不绑定目录；之后最多绑定一个本地文件夹。任务通过项目 ID 归属项目。文件夹仅作为项目的本地执行边界，不复制内容，也不替代项目身份。
```

影响：

1. 覆盖原“左导航从本地文件夹开始”的 Locked 表达，三栏结构保持不变。
2. 内部继续使用 Workspace 作为项目级作用域，避免破坏策略、记忆、事件和产物隔离键。
3. Workspace 的目录绑定改为可选，并新增显式绑定命令；已有数据兼容为“已绑定目录的项目”。
4. 左栏统一为项目/任务层级，使用正式项目创建表单和系统目录选择器。
5. 一个目录不能绑定到多个项目；本轮不支持一个项目绑定多个目录。
