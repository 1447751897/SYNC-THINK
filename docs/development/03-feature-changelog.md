...
- 模型服务 overloaded 时，执行过程底部不再写「运行失败」加英文原文。标题改为「回答中断」，摘要改为「模型服务繁忙，回答没有写完。」原文仍在展开详情里。
- 回答里被截断的 `![...](sync-think-image://generated/…)` 会用工具结果补全成真正的图片；补不全则隐藏，避免把本地协议 URL 显示成一串明文。
- 生图指引改为：执行过程卡片已经展示图片，回复里不要再粘贴 `sync-think-image://` 链接。

### Verification

- Desktop 定向测试：`markdown-image-gallery.test.ts`、`MarkdownContent.test.tsx`、`ChatView.terminal-error.test.tsx`。
- Runtime 定向测试：`generate-image-tool.test.ts`。

## 2026-09-10：模型设置去掉成功 toast 并换掉原生下拉

### Changed

- 创建供应商、图片识别 Fallback、规划 & 执行、云同步成功后不再弹出右上角提醒；结果已在页面上，失败仍报错。
- 视觉模型与规划/执行/思考强度下拉改为 overlay 列表：关上保持圆角触发器，打开后 12px / 32px 行，当前项打勾，不再使用系统原生 `<select>`。

### Verification

- Desktop 定向测试：`ModelSettings.test.tsx` 中 Fallback / 规划执行 / 云同步保存后无 toast，下拉为 listbox。
- 页面步骤：设置 → 模型，创建或开关 Fallback / 规划执行后确认无成功条；打开视觉模型和规划模型列表，核对其为设置页浮层而非系统蓝条。

## 2026-09-10：模型列表停用区与三点菜单

### Changed

- 设置 → 模型左侧「已停用模型」改为区块头：计数徽章、Chevron 开合，名单在触发条下方展开，不再用上浮 popover，也不再扮成选中的供应商行。
- 启用行、停用行和停用区块的三点菜单统一为 overlay 浮层：12px 字、32px 行高；「移除」用错误色并二次确认为「确认移除」。图像生成左侧同一套控件一起改。

### Verification

- Desktop 定向测试：`ModelSettings.test.tsx` 中停用区展开与选中后保持展开。
- 页面步骤：打开设置 → 模型 → 文本生成，展开已停用模型、核对行悬停三点菜单与停用区块头。

## 2026-09-10：请求日志去掉展开并按上报显示缓存明细

### Changed

- 设置 → 模型 → 使用统计 → 请求日志不再提供左侧 Chevron 和展开后的请求信息 / Token / 费用明细。Token 列只保留总数与空心圆悬停明细，费用列只显示总额。
- Token 明细顺序为输入、输出、缓存读取、缓存创建、总 Token。未上报的缓存字段整行不渲染，不写「未上报」；已上报的 `0` 仍显示 `0`。
- 行悬停对整行使用同一背景；空心圆悬停只加深边框、保持透明，避免 Token/费用列出现色块或实心圆。

### Verification

- Desktop 定向测试：`ModelSettings.test.tsx` 中缓存已上报与未上报两条请求日志用例。
- 页面步骤：打开设置 → 模型 → 使用统计 → 请求日志，核对无展开箭头、行悬停均匀、空心圆悬停出 Token 明细且未上报缓存字段不出现。
...
- 新增 `apps/website` 官网与账号页面：原创绘画雪山背景、Instrument Serif 品牌标题、玻璃页脚、下载入口与响应式布局。视觉方向结合 Multica 和用户提供的 Nexeus 参考，背景交付为约 350 KB WebP。
- 首页嵌入独立 `/demo` 交互演示，支持需求、计划审批、执行步骤、工具详情、产物查看和重置；使用示例数据，适配手机，支持复制 iframe 代码。修复实际 iframe 沙箱中发送按钮失效的问题。
- 新增 `apps/cloud` Node.js + Better Auth + SQLite 账号服务：邮箱注册、验证、登录、找回密码、会话和退出；包含生产配置校验、接口限流、受保护账号页与按来源限制的演示嵌入。

### Verification

- 官网 12 项与账号服务 15 项测试通过，包含实际沙箱 iframe 回归；构建、类型检查和定向 lint 通过。
- Edge 实测注册、邮件验证、登录、退出、密码重置与刷新后继续重置；测试邮件由隔离测试环境捕获。桌面与手机浏览器检查了图片、换行和完整演示流程。
- 本地预览已启动。正式服务器的域名、HTTPS、SMTP、备份与运维配置仍待落地；桌面账号绑定、跨设备同步和云端 Agent 执行属于后续阶段。部署清单见 `docs/engineering/17-cloud-account-launch.md`。

## 2026-09-06：开头页改成对话空态

### Changed

- 没有工作区时不再铺步骤卡、内核安装和说明文字。开头页使用与对话空态相同的问候语和 Composer；点输入框即可打开工作区。
- 已打开工作区但还没有模型时，只保留一条与提示条同形的「添加 API 密钥」。内核安装仍在关于页。

### Verification

- 定向回归：`FirstLaunchGuide` 6 项、`EmptyTalk` 开头页 2 项、Phase3 welcome 11 项、`ShellApp` 72 项通过。Desktop `typecheck` 通过。

## 2026-09-06：公开 Beta 0.1.0-beta.1 无签名分发

### Changed

- 版本升为 `0.1.0-beta.1`。首次启动补上「添加 API 密钥」；Pi 不再出现在首次引导、关于页和工具栏的安装入口，打包后首次启动也不会自动安装 Pi。
- 关于页在未配置更新通道时写明「当前 Beta 为手动下载」。
- 新增 `docs/operations/beta-public/` 下载页模板与测试说明；构建后由 `scripts/assemble-beta-public.mjs` 写入 `apps/desktop/release/public/`。本轮不启用自动更新源。

### Verification

- 定向回归：Desktop 首次引导/Pi/关于页 82 项，另 ShellApp/Settings 126 项通过；`kernel-update-wiring` 2 项确认 bootstrap 跳过 Pi。Portable verify 无 forbidden files。Installer verify schema v3、`signing.mode=unsigned-fixture`。当前重发包 `SYNC-THINK-Setup-0.1.0-beta.1-x64.exe` 317,718,294B，SHA-256 `49d3bd8ecf98a8ab28b178c86371a34350c0c13c5145b571848f953052556d33`，含精简开头页。
- Desktop `build` 通过，首屏 JS 1,926,428B，全部 2,749,185B。

## 2026-09-06：浏览器点击识别文本定位器，网关 502 不再立刻换模

### Changed

- `browser_click` 接受可见文案和 Playwright 的 `a:has-text("...")` / `text=...`。内嵌页先解析可见目标，再由主进程 `sendInputEvent` 发出可信点击；点不中时工具返回失败，并带上附近可点文案。
- `browser_read` 优先汇总可见链接，避免首页前 40 个隐藏/导航链把游戏入口挤掉。
- 本地网关 502/`fetch failed` 先重试当前模型；不再因为同一供应商的另一个模型去消化传输故障。若仍切换，状态行会写出「网关 502」这类原因。

### Verification

- 定向回归：Shared 定位器 5 项、Core 换模 27 项、Runtime 浏览器/fallback 83 项、Desktop 命令桥 18 项通过，覆盖 `a:has-text(...)` 不进 `querySelector`、点不中返回失败、主进程可信点击坐标，以及本地网关 502 先重试当前模型。Shared / Core / Workers / Runtime / Desktop `typecheck` 通过；Desktop `build` 通过，首屏 JS 1,930,486B，全部 2,752,855B。

## 2026-09-06：执行活动行对齐 Beautiful UI Drive / Churning 动效

### Changed

- 底部活动行的 `3 × 3` 像素格改为 Beautiful UI Drive 的全亮平台：暗态 `0.15`，`18%–42%` 保持全亮，周期 `650ms`、`ease-in-out`，格距 `1.5px`。错峰延迟本来就已经是 `(列 + |行 - 1|) × 90ms`。
- 活动文案和流式 Think 扫光改为次要色→主文字色的 `90deg / 1.4s` 墨色扫过，不再用强调色高亮条。审批等待仍不加扫光。产品文案不改成 `Churning`，也不引入 Surfer 或 Agent Screen。

### Verification

- 定向回归 3 个文件、73 项通过，覆盖 Drive 9 格错峰延迟、`pixel-on` 全亮平台、墨色扫光，以及执行过程活动行仍在底部。Desktop `typecheck` 通过；`build:shell` 通过，首屏 JS 1,926,306B，全部 2,748,675B。

## 2026-09-06：MCP 卡片识别品牌图标并区分可调用状态

### Changed

- 「我的 MCP」不再一律画通用 Server 图标：GitHub 用品牌标记，Context7 等远程地址用站点 favicon。
- 启动命令如果还是 `MCP_GITHUB_COMMAND` 这类未展开环境变量，卡片显示「命令未配置」，不能当作可调用。刷新/调用时会先展开 `${VAR}` / `%VAR%` / 裸环境变量名，并允许 NewMax 常用的 `uvx`/`bunx` 与已展开的本地可执行文件。

### Verification

- 定向回归：Desktop 图标/可调用 45 项、Workers stdio 14 项通过，覆盖 GitHub/Context7 图标、`MCP_GITHUB_COMMAND` 未配置不可调用，以及环境变量展开后按 npx/uvx 启动。Desktop `typecheck` 通过；`build:shell` 通过，首屏 JS 1,926,306B，全部 2,748,675B。

## 2026-09-06：Skill 常驻上下文按 NewMax 统计 description 字符

### Changed

- 「我的 Skill」常驻上下文占用改为加总已启用 Skill 的 frontmatter `description` 字符，建议上限 15,000 字符；不再把从未触发过的历史 `contextTokens` 当成占用。
- 行内「已扫描」表示描述健康检查通过；缺描述或超过 1,536 字符分别显示「缺描述 / 描述截断」，并计入「有问题」。全局范围统计全部启用项，工作区范围只统计该工作区已激活项。

### Verification

- 定向回归 2 个文件、45 项通过，覆盖 description 字符预算、缺描述/截断扫描，以及未使用 Skill 在 `contextTokens = 0` 时仍显示非零常驻占用。Desktop `typecheck` 通过；`build:shell` 通过，首屏 JS 1,925,851B，全部 2,745,700B。

## 2026-09-06：失败命令收起、排队不打断、浏览器滚轮缩放

### Changed

- 连续命令段和外层执行过程在结束后无论成功或失败都收起；失败详情仍可手动展开查看。
- 运行中把下一条需求放进队列不再触发延时自动发送，因此也不会把上一轮标成「回答已中断」。只有当前回复结束后才按 FIFO 派发，或用户主动点「插话」。
- 内嵌浏览器支持 `Ctrl`/`Cmd` + 滚轮缩放，步进与工具栏加减号相同。

### Verification

- 定向回归 8 个文件、139 项通过，覆盖失败段折叠、排队 7 秒仍不派发、以及浏览器 viewport / guest 滚轮缩放。Desktop `typecheck` 通过。

## 2026-09-06：对话标签按指针槽位让位，一轮可打开多个网站

### Changed

- 对话标签重排改为指针槽位：被拖标签跟随指针，目标槽不再随「鼠标下方那个标签」来回翻转；松手按槽位提交顺序。
- 同一轮里多条成功的 `browser_open` 会分别打开或聚焦对应网址，不再只留下最后一条。

### Verification

- 定向回归：对话标签 22 项、槽位几何 6 项、ShellApp 72 项通过，覆盖指针槽位不翻转、松手仍提交顺序，以及一批两次 `browser_open` 打开两个工作台网页。

## 2026-09-06：对话标签让位动画，并修正打开网站覆盖已有页

### Changed

- 对话 Pane 资源标签在条内拖动重排时，邻签按与工作区标签相同的 `visualIndexFor` 即时让位；跨 Pane 分屏仍使用原生拖放，不引入 `motion`。
- `browser_open` 不再改写已打开的不同网站：Pane 里的 GitHub 等页面保持原址，新 URL 在右侧工作台新开一页；同一 URL 只聚焦已有工作台标签。
- 打开网站不再默认把「工作区文件」停靠到右侧；文件/审阅/终端仍会带上文件栏。

### Verification

- 定向回归 7 个文件、185 项通过，覆盖工作台按 URL 开页、命令桥不劫持已有 guest、对话标签拖动让位，以及 Shell 在已有 GitHub Pane 页时只新开 4399 且不带文件栏。
- Desktop `typecheck` 通过。Electron 实窗需重启桌面端后：拖动对话标签确认让位；已有 GitHub 页时再说「打开网站 4399」，GitHub 应保留、只新开一页 4399、右侧不自动出现工作区文件。

## 2026-09-06：工作区文件夹标签按 beUI Morphing Tabs 滑动与让位

### Changed

- 顶栏工作区标签改为 beUI Morphing Tabs 的指针槽位：点击切换时共享 NewMax 曲面滑到目标槽，拖动时邻签即时让位，松手后提交顺序。
- 去掉工作区标签上的 HTML5 `draggable` 与 dnd-kit 双轨拖拽；Pane 资源标签仍用原生拖放。不引入 `motion` 包。

### Verification

- 定向回归 4 个文件、72 项通过，覆盖槽位几何、共享曲面、拖动让位、顺序提交，以及关闭界面动画时取消位移过渡。
- Desktop `typecheck` 通过；未引入 `motion` 包。Electron 实窗需重启桌面端后点选/拖动工作区标签确认。

## 2026-09-06：对话文件、审阅与新建文档按 NewMax 一次展示

### Changed

- 工作区「对话文件」和独立审阅自动拼齐目录/文件分页，不再显示「下一页文件 / 文件 1–N」；空态改为「当前对话暂无文件变动」。聊天单轮文件卡仍保持原分页。
- 新建文档立即写入空文件再打开，保存控件改为 NewMax 的「保存 / 保存中 / 已保存」，并提供自动保存。富文本工具栏补齐撤销/重做、H2/H3 和有序列表，不引入新编辑器引擎。

### Verification

- 定向回归 7 个文件、77 项通过，覆盖对话文件/审阅自动拼齐、聊天单轮文件卡仍分页、空态文案、新建文档落盘与保存控件文案。
- Desktop `typecheck` 通过；`build:shell` 通过，首屏 JS 1,920,195B，全部 2,738,613B。

## 2026-09-06：执行步骤与命令结果按 NewMax 一次展示

### Changed

- 执行过程自动拼齐步骤分页和时间线分页，不再显示「下一页步骤 / 继续加载」；文件变更分页保持原状。
- 展开工具后自动读取并拼接完整输出，去掉「读取完整内容 / 上一段 / 下一段」。超长结果只保留 NewMax 同款本地截断提示，复制仍取已拼齐全文。助手正文的按需原文读取不变。

### Verification

- 定向回归 11 个文件、140 项通过，覆盖工具输出拼接、步骤累积、时间线自动续页、事件详情展开即读，以及文件差异「读取完整内容」仍按需分页、不自动预取。
- Desktop `typecheck` 通过；`build:shell` 通过，首屏 JS 1,918,212B，全部 2,736,630B。

## 2026-09-06 当前任务清单与左侧导航固定

### Changed

- 移除 ChatView 的“历史任务”入口和面板挂载，不再发起其历史查询；保留后台原生事件和持久化数据。
- 当前清单至少有一项未完成才展示；整单完成或为空时隐藏，包括重启恢复的快照。仍保留进行中清单内的已完成子项、说明及进度，不把模型单轮结束误当任务完成。
- 导航由消息滚动区改为聊天列绝对定位，刻度监听轨道自身尺寸；展开、收起及任务完成不再移动导航。补充左侧内容间距和壁纸主题定位规则。

### Verification

- 三项新增回归先失败、修复后通过；任务、滚动锚点和导航相关 7 个文件共 88 项定向测试通过。Desktop 生产构建通过；定向 ESLint 无错误，ChatView 16 项既有 Hook 提示保留。
- 现有 Playwright + 真实 ChatView/生产 CSS 隔离验证 1366x900、820x760、390x844：每例 60 个刻度，展开/收起/任务完成最大位移 0px；点击跳转、预览、窄屏无横向溢出、壁纸定位通过，页面与控制台错误为 0。
- Claude/Codex 原生快照恢复均验证整单完成隐藏；Claude 未完成跨轮保留、Codex 新轮清空，历史任务请求为 0。使用受控事件与快照，不新增实际模型调用。
- 截图、脚本与坐标证据位于仓库外 `C:/Users/zhuzhenyu/AppData/Local/Temp/sync-think-current-tasks-ZRaDEq/`。
- Desktop 全量复验：258 个测试文件、2122 项测试全部通过；生产构建产物首屏 JS 1,915,098B。未扩展后端或性能优化范围。

## 2026-09-06 收尾：执行结果真实性与失效步骤

### Changed

- 文件变更仅在已成功执行的写工具结果上生成，兼容Claude原生Write/Edit大小写；过滤等待、拒绝、取消、失效和失败，保留读取/命令原始输出但不猜测本轮文件变更。
- 原生工具ID与审批请求ID分开；共享失败判定与精确审批关联，支持旧Claude唯一参数匹配和Codex itemId。存储过滤事件、实时快照、历史目录版本及恢复时间线同步终态，完成汇总排除未成功工具。

### Verification

- Shared72 / Storage502 / Runtime1497 / Desktop2119，四包全量4190项通过；11项构建、lint、diff-check通过，18项既有Desktop Hook提示保留。
...
- 待审批采用独立静态“等待你的批准”活动，不误判stall；离开审批状态后重置无进展计时，支持重开的待审批窗口。
- 普通user消息在既有事务中保存本轮runId；旧NULL行按唯一、同workspace/thread/run的连续事务事件只读关联。恢复保留现有草稿，不重用旧批准、不自动发送；明确其它run和歧义拒绝关联。
- 内核checking与installing/verifying分开，已安装内核检查更新时保持可选。

### Verification

- 新增18项回归；Storage501/Runtime1469/Desktop2089三包全量4059项、lint与11项构建通过；未变Protocol113/Shared62沿用，累计4234，Desktop18项既有Hook提示保留。首屏JS1,920,706B、全部2,739,124B，各+607B。
- 实际生产Electron、三内核和真实配置模型共12个按钮场景通过；原失败旧数据库精确原文恢复、现有草稿合并、NULL行不回填亦通过。初始隔离内核目录不一致/DOM文本断言问题留证，不冒充生产修复。
- 08:19:20Z成套重启至Desktop94292/Runtime94336，inFlight0，业务960/34/31、5个迁移对象和原5项历史保持。无新迁移、清理或installer。
- 新发现的未执行文件被统计为已修改、失效后过程活动终态仍待修复；完整六方向继续，见审查18.20、TD080和本地开发28节。

## 2026-09-06 审查续记：三内核真实审批与关闭收尾

### Changed

- 原生审批请求登记失败时拒绝本次工具操作并返回，不再创建缺少持久记录的隐形 waiter；不扩大授权、不自动重试。
- Runtime统一拥有Native/Claude/Codex完整执行Promise，关闭时先禁止新运行并等待取消收尾，再关闭持久数据库；关闭后目标续轮不读库。

### Verification

- 6项SQLite故障/关闭顺序回归先红后绿；最终Runtime197文件/1468项全量、47项相关、19项关闭兼容、6项终检、lint和11项构建通过。其余四包未变基线沿用，累计4216，非本批五包全量。
- 真实Native/Claude/Codex各5场景：pipe重连批准、拒绝、待批取消、正常关进程重开、强制终止自有Runtime并新PID同库重开，全部通过；迟到授权不执行，显式取消与expired分开。非实际Electron点击或完整生命周期矩阵。
- 06:41:58Z成套重启，Desktop92580/Runtime39392健康、inFlight0；业务960/34/31及原5项历史保持。JS首屏1,920,099B/全部2,738,517B不变，无迁移、清理或installer。Native早期同会话工具空转观测保留；完整六方向继续，见审查18.19、TD079。

## 2026-09-06 审查续记：三内核真实计划与Codex工具启用

### Changed

- Codex受管thread/start与thread/resume显式启用tools.update_plan.enabled，修复新版实际目录没有原生计划工具、只回答完成却无事件的问题。保留用户全局配置与goals，不用平台MCP伪装原生计划。
- 持久化自检分项报告会话/记忆/进程/工具/用量；reasoning独立观测，显式E2E_EXPECT_REASONING=1才设为必需。自定义提示的未测工具明确not-requested。

### Verification

- 实际Native/Claude Code/Codex共9轮，详细原生任务、状态、同会话下一轮、Runtime对象重开回看和口令保持通过；另通过旧失败Codex原sessionId升级。真实上游目录与事件证明修复，不是只有模拟通知。
- Runtime1462项全量、31项定向、lint及11项构建通过；实际Codex自检同thread跨2进程通过。其它四包基线未变，累计4210，非本批五包全量。JS首屏1,920,099B/全部2,738,517B不变。
- Desktop90236/Runtime86068成套启动，05:29Z健康，业务960/34/31保持。无迁移、清理或installer；审批/图片/取消/失败等及完整六方向继续，见审查18.18。

## 2026-09-06 审查续记：原生任务历史回显

### Changed

- 当前任务与历史快照分开：新增按需只读历史入口，成功原生结果确认状态，保留完整描述、Claude跨轮更新与明确清空；历史“进行中”标记为记录时状态，不自动执行。
- 独立Worker读取接口，严格会话归属、10轮目录/40项与224KiB集合分页、复合游标和版本校验。缓存有界且检测事务/外部变更；不同会话与过期响应隔离。没有新增生产依赖或迁移。
- 修复后原5项测试清单通过真实业务pipe回显；源记录没有描述的项保持事实，不补写内容。

### Verification

- Shared62/Protocol113/Storage493/Runtime1454/Desktop2080共4202项全量通过，11项构建、Runtime/Desktop lint及diff-check通过；Desktop18个既有Hook提示保留。JS首屏1,920,099B/全部2,738,517B。
- 真实Electron IPC、生产Main/Preload/Renderer与Runtime/只读Worker、隔离SQLite实测40+5项分页、12轮目录、清空、跨会话、820/1280px浅深色与成套重启恢复，页面/控制台错误和警告0。非真实三内核执行矩阵。
- 2026-09-06T04:26:10Z，Desktop85260/Runtime68032成套启动且健康，业务960/34/31保持；原清单5项成功从新API读出。无新迁移、清理、重复17GB备份或installer。原六方向继续推进，见审查18.17。

## 2026-09-06 审查续记：失效审批与原请求恢复

### Changed

- 审批列表返回持久失效原因、最近20条无参数摘要和总数；旧审批决定幂等返回expired，不把过期deny伪装为批准成功。
- 原请求按thread/run唯一user消息索引定位，原文按需完整读取；图片经Main校验会话/消息/run/image归属并受限读取，保留原技能选择。缺失或错配明确报错，不使用最新消息替代。
- 失效提示仅提供重新编辑，保留当前草稿/附件/技能；切会话或读取失败不泄露旧稿，不自动发送或沿用旧工具授权。

### Verification

- 全量4164项通过（Runtime1450/Desktop2075/Storage488/Protocol95/Shared56）；最终22项定向复验通过。Storage首次并行全量中的既有5秒回滚测试超时与EBUSY日志保留，未改测试，串行全量488通过。
- 工作区11项构建及Runtime/Desktop lint通过，Desktop18项既有Hook提示；最终首屏JS1,912,981B、全部2,731,399B，预算通过。
- 实际ChatView/生产CSS/SQLite/Runtime/Main图片边界、合成IPC实测恢复与显式发送、缺图、切会话、820/1100px和浅/深色；最终页面错误与资源失败0。真实Electron IPC/内核矩阵仍单列。
- 新Desktop77880/Runtime66672于本机成套启动，02:44Z健康，02:47Z业务计数保持960/34/31；无新迁移、物理清理或installer发布。历史任务回看等完整六方向仍在推进，见审查18.16。

## 2026-09-06 审查续记：插队原子交接与本机成套切换

### Changed

- 新指令准备/落库失败不提前停止旧运行；旧 partial history/timeline、新消息、版本和审批/取消/启动事件同事务，成功后才兑现abort。准备后复查任务/版本并重取投影；严格写入与指纹缓存避免回滚后的假成功。
- 既有0052/0053/0054已应用到本机17GB库，最终停写点备份保留；新Desktop77300/Runtime31156已经启动。没有installer发布、数据物理清理或新增生产依赖。

### Verification

- 本批Runtime193文件/1444项全量、21项定向真实SQLite故障注入通过；typecheck/build/lint及工作区11项build通过。其它包沿用前批，累计基线4140，不混称本批全量重跑。
- 第一份17,007,509,504B备份完成integrity_check=ok、SHA-256和82表计数；第二份停写备份完成SHA-256/旧版存储读取验证，事件高水位1993644及业务计数保持。独立1e1427a构建与空库worker通过，不等于完整业务/旧Desktop回退。
- 实际生产壳设置往返通过，6条历史消息加载通过；任务卡等待失败已保留：后续Codex run.started清空非Claude当前计划，历史5项成功事件仍在。当前/历史任务边界、真实内核矩阵和正式更新回退继续见审查18.15，目标保持进行中。

## 2026-09-05 审查续记：输入持久化边界与失败草稿

### Changed

- 精确校验完整消息块 JSON 的 256KiB 字节预算，保留 100,000 UTF-16 单元入口；在运行准入前拒绝超限文本和技能快照，不截短执行。
- append 最终消息与任务版本/新事件同事务写入，严格传播存储失败；普通发送失败恢复草稿，新编辑与异会话隔离，支持返回原会话后手动追加恢复。
- 复用存储校验与文本块构造，助手兼容降级不变，无新增迁移/依赖。草稿未扩展为重启持久化，旧运行提前取消边界仍独立待验。

### Verification

- 4125 项全量通过：Runtime 193/1429、Desktop 253/2058、Storage 47/487、Shared 14/56、Protocol 21/95。构建/typecheck/lint 通过，Desktop 19 个既有 Hook 提示。
- 真实 ChatView + 合成 IPC/生产 CSS 完成草稿失败与会话隔离验证，820/1100px 无横向溢出、页面错误 0；生产首屏 JS 1,906,351B、全部 2,724,769B。不是业务窗口/实际内核证明。
- 本批未切换业务窗口、迁移/备份或清理业务历史；审查 18.13 和本地开发第 22 节记录证据、边界及后续门禁。

## 2026-09-05 审查续记：历史原文恢复与来源缓存

### Changed

- 旧 canonical metadata 和 text-only 最终消息通过身份/前缀核验恢复精确原文引用；新增受限 event-prose，保留私有字段与会话隔离，新引用不重复扫描，不改旧消息记录。
- 分段原文准备结果使用 8 项/16MiB 字符串估算 LRU，保留原始 UTF-16；数据库变更、DDL、读中竞态、外层事务回滚均安全失效。首次和容量外来源仍有全文准备成本。
- 导航 RPC 测试等待真实解码响应，设置测试等待并限定弹窗内控件，不用增加套件超时掩盖抢跑。

### Verification

- Runtime 192/1423、Desktop 252/2051、Storage 47/485、Shared 14/56、Protocol 21/95 全量通过，构建/lint 通过（Desktop 19 个既有 Hook 提示）。首屏 JS 1,904,774B、全部 2,723,192B。
- 真实旧消息 UI 与管道重启/版本/作用域验证；同源缓存 20 段 p95 0.243–0.294ms，事务绕过约 20.605–23.049ms，暖态复制各一次约 210–216ms。只代表隔离样本，证据与边界见审查 18.12。
- 本批未切换业务 Runtime、迁移或回收历史，完整目标继续。

## 2026-09-05 审查续记：长正文原文回显与完整消息操作

### Changed

- text/thinking 增加 canonical 引用；普通 message 正文按原字段引用。兼容截短不再丢失完整来源入口，实时长 textDelta 与重连重复字段收敛，保持原存储/帧守卫。
- 聊天回答、用户正文和过程思考默认零完整读取，点击后明确显示折行原文分段。复制/重新生成按完整来源和同一版本读取，带失败反馈、重入限制、会话切换取消，并避免权威 timeline 与兼容块重复拼接。

### Verification

- 全量 Runtime 192/1421、Desktop 252/2051、Storage 46/470、Protocol 21/95、Shared 13/53；构建/lint 通过，Desktop 19 个既有 Hook 提示。生产首屏 JS 1,904,595B、全部 2,723,013B。
- 真实唯一管道重连/重启续读与跨会话拒绝、实际 ChatView 的 3.20MB 完整复制与 198KB 原提示重发、503 重试及 820px 无溢出通过。日志/截图/性能样本与遗留来源/预算边界见审查 18.11。
- 业务 Runtime/数据库未切换或迁移，完整优化目标继续；开发验证不等于成套发布。

## 2026-09-05 审查续记：未知大事件展示与 Pi 执行门控

### Changed

- 未知大事件采用有界展示与 event-display 公共来源，实时/回放不再因未知大字段缺失；SQL 与读取共享公共 Run 白名单，长度一致，私有状态不进入展示来源。修复嵌套长键预算和字段顺序挤掉路由/失败标记的问题。
- 工具步骤新增 detailsRef，原始展示数据按会话/版本分段读取，展开不自动加载完整内容；保持 1MiB 限制和原始持久记录。
- Pi 安装/版本与执行能力分开：显示未接通、禁用选择，旧会话和首轮保留草稿；Runtime 对消息、Run 准备、目标启动/恢复前置校验。不添加 Pi 适配器，不静默换内核。

### Verification

- Runtime 192/1416、Desktop 250/2032、Storage 46/470、Protocol 21/95、Shared 13/53 全量通过；构建和 lint 通过，Desktop 19 个既有 Hook 提示。生产首屏 JS 1,900,249B、全部 2,718,667B。
- 真实管道重启续读和精确作用域、实际工具卡/ChatView 的 820px 读取与草稿通过。隔离 3.20MB 事件变为 21,848B，投影 p95 7.47ms、暖段 p95 21.90ms；每段仍解析完整来源，不是整窗指标。
- 业务 Runtime 和数据库未切换/迁移，完整优化目标继续；全响应预算、非工具正文、后续性能/审批 UI 和发布矩阵见审查 18.10。

## 2026-09-05 审查续记：会话文件目录与单轮分页整合验证

### Changed

- 默认文件审阅采用 conversation.listFileChanges，从完整持久会话读取，不再沿用最后一轮的文件前缀/游标。单轮入口保持原语义，标签键隔离，重开会话审阅可恢复目标。
- 同路径按工具事件持久顺序保留最后变动，文件附带 Run 来源与精确正文引用。40 项/224KiB 双限制，Worker 目录缓存最多 8 项/16MiB JSON；可见页才读取，版本保护与手动重读/重试共用既有 3 槽读取队列。
- 新增 0054 会话起点/Run 游标索引，查询计划与迁移回归通过；业务库尚未应用。

### Verification

- 全量 Runtime 191/1406、Desktop 249/2028、Storage 46/469、Protocol 21/95 通过。真实管道/Worker 跨重启续页、会话与工作区隔离、读取中重绑、跨轮晚写覆盖通过；实际组件两轮 200 文件完整读取，40 行挂载、820px、503 重试、版本旧页保护通过。
- 构建首屏 JS 1,898,701B、全部 2,717,119B，通过预算；19 个既有 Hook 提示保留。首次完整运行的既有测试超时/异步竞争及新夹具错误预期保留日志，按项目 15 秒脚本复跑通过。
- 当前只完成开发验证，不代表已发布。全响应异常大字段预算、其它性能/能力/审批 UI 项与 0052/0053/0054 备份迁移、真实内核矩阵继续推进；详细证据见审查 18.9。

## 2026-09-05 审查续记：总过程分页实施中，待补会话汇总来源

### Changed

- 步骤/文件/旧任务集合改为 40 项、224KiB 双预算首屏与有版本续页；完整命令保留原参数来源，最新活动和全量统计不随分页丢失。分页带原会话作用域，复用只读 Worker 与三个真实在途槽位。
- Worker 过程快照缓存按游标失效、按数量/JSON 字节淘汰；摘要复用，正常翻页不重复重建全程。通用请求队列从工具模块独立出来。
- ChatView 回退过程、单轮文件卡和单轮审阅接入页控、版本恢复与手动重试。修复工具展开状态跨页串用、失败过程自动收起、终态统计只剩当前页和审阅差异列错位。

### Verification And Remaining Integration

- Runtime 1402、Protocol 94、Storage 467 全量通过；Desktop 2024 项在 2 Worker 全量通过，默认并发的既有 ShellApp 懒加载竞态另有失败记录。构建/lint/tokens 通过，详见审查 18.8。
- 实际 ChatView / 单轮审阅 + 独立 SQLite/Worker 验证 5000 命令与 120 文件，当前页 40 项、首屏帧 23,711B；20 次暖页 p95 约 2.86ms，不是整窗指标。
- 已复现的未收尾项：会话汇总函数合并多轮文件后错误继承最后一轮游标。下一步提供会话级文件目录来源与分页，验证同路径后写覆盖、多轮历史和重启，之后再收尾。未部署业务窗口或迁移/清理用户数据。

## 2026-09-05 审查续记：方案优化第九批——文件差异按需读取

### Changed

- fileChanges 前后大字段改为原事件精确引用，支持 JSON 参数/args 与写前快照；已知空文件侧保持空字符串，替换片段不冒充完整文件。完整原始正文保留。
- 新增会话作用域 conversation.readFileDiff，经只读 Worker 在同一事务读两侧来源；组合/单侧版本约束续页与长行跳读。默认 80 行、上限 160 行/页，每行 512 单元；精细比较超预算明确按整段替换。
- 聊天文件卡/右侧审阅共用组件和三槽读取池，不自动预取，来源绑定原会话。版本变更保留旧页、失败重试、前后原文分段读取；未知统计显示按需计算。补齐 600 行短文本旧降级路径，并修正行号列换行。

### Verification And Limits

- 日志本地时间 2026-09-06：Runtime 1395、Desktop 2017、Storage 467、Protocol 92、Shared 46 项全量通过；构建/typecheck/lint/tokens/diff 检查通过。实际 Edge 聊天/审阅、只读 Worker、独立 SQLite 与真实 Runtime 重启测试通过，详见审查 18.7。
- 三文件含万行及两侧各 1.2M 字符的过程帧 26,359B；80 行 DOM 与 16384 字符单文本节点验证有效。测试没有提升协议帧上限或删除历史正文。
- 每页仍读取/解析/哈希原始来源；总帧预算、未知字段/普通长正文、底层去重、流式完整导出和整窗性能仍待治理。未部署用户窗口或迁移/清理实际库；完整目标保持进行中。

## 2026-09-05：方案优化第九批——工具完整内容按需读取

### Changed

- 复用 event/message/timeline 原始持久内容，新增 conversation.readContent 与会话/线程/工作区前后校验；只读 Worker 提供默认 16384、最大 32768 UTF-16 单元的无损分段，续页必须保持 SHA-256 内容版本。没有新增依赖或数据库迁移。
- 实时事件、重放、消息页和工具时间线发送轻量预览与来源引用，恢复用完整 run/增量不进入展示帧；文本、数字、键名和集合均受预览预算约束，保留错误状态与真实列表总数。消息单条 256KiB 校验保持。
- 真实工具卡默认不读取完整内容；点击后支持前后段、从头读取、失败重试、会话切换取消，复制按钮区分预览/本段。单文本节点和单片段状态约束 Renderer，复用历史过程三个真实在途槽位并优先用户操作。
- 稳定工具片段投影缓存检测来源字段/运行变化；修复短多行/摘要截短无完整入口，以及按需内容分页栏被原外层滚动容器裁切的问题。

### Verification And Limits

- 工具读取批次收尾复现独立 fileChanges.content 超帧（审查 18.5），现已通过后续差异引用补齐（18.7）；总帧预算等剩余项继续推进，不将单链路修复视为全部大帧治理完成。
- 存储、协议、RPC/Worker/真实 Runtime 重启、IPC 契约与 UI 回归；Edge 实际组件检查 1280×900、820×740 的正常/错误/恢复路径。详细最终计数、截图及基准口径见审查第 18 节。
- 隔离样本 2.64MB 事件 JSON → 6.02KB，10 条合法消息页 2.21MB → 23.9KB；这是传输投影。每段仍读取/解析/哈希整个源记录，底层去重、流式导出、普通长正文、整窗性能与真实内核发布矩阵继续推进。
- 未部署用户窗口、未迁移实际库或物理清理历史；前批新增量格式仍要求成套更新、备份与兼容恢复。

## 2026-09-05：方案优化第八批——Native 增量状态与事务恢复

### Changed

- 新增有版本/摘要/身份/结构校验的运行状态增量与独立已提交基线，减少 Native 非终态、fallback 与重试事件重复携带的累计内容。首条状态、小状态、无收益变化以及完整检查点继续保存完整内容。
- 每次完整检查点和进程重启后重建基线，终态释放；SQLite 内层与外层 UnitOfWork 失败同时保留正确持久基线/检查点游标。操作值独立复制，避免消费端修改事件污染后续恢复。
- 复用已提交状态摘要、单次准备当前快照，正常追加工具历史不反复扫描前缀；过程/任务/审批专用投影去掉恢复增量，run/thread/model 元数据保持。协议新增可选 runtime.incrementalRunState，无 schema 迁移或新增依赖。

### Verification

- Runtime 185 files / 1376 tests、Storage 45 files / 459 tests、Protocol 17 files / 88 tests、Desktop 相关 75 项、新增专项 20 项通过；Runtime 构建、定向 ESLint、差异检查通过。
- 独立 SQLite 132 事件的三次交替对照，包含第 128 条完整检查点：累计文本/工具写入状态字节减少约 96.62%/96.11%。长工具数组 20000 条正常追加的独立增量计算约 40–43ms（初版 238–247ms），不含摘要准备和数据库。
- 真实 Runtime+文件数据库验证重启恢复、模型回退、跨运行瞬态、SQL/外层事务失败与旧完整记录兼容；初版外层回滚用红灯复现后修复。增量校验使强制无检查点全重放工具样本更慢，未以写入收益掩盖该代价。
- 详情、基准口径和证据见审查第 17 节、TD-066、本地开发第 14 节。业务窗口未切换；旧 Runtime 不识别新增量，版本回滚必须配套经过验证的兼容备份。单次大结果引用、页面/整窗性能及真实内核矩阵仍继续推进。

## 2026-09-05：方案优化第七批——完整历史目录与按需锚点读取

### Changed

- 新增只读 conversation.listNavigation 与 aroundMessageId 消息页，经 Worker/Runtime/IPC 回显完整历史目录；摘要有界，跨线程锚点和读取期间失效的会话被拒绝。
- 正文继续按页读取并保持已读组件挂载，稀疏区间明确显示加载入口；最新页按覆盖范围替换过期行、保留更早已读页面。100 条会话缓存裁剪时同步范围和游标，避免重开后跳过历史。
- 跨页跳转、补页和会话切换校验旧响应；读取提示移出正文流，失败手动恢复不重跑工具，保留代码展开与横向位置。历史回填终态元数据维持导航配对顺序。

### Verification

- Desktop 242 files / 1999 tests、最后 27 项定向回归、Runtime 相关 46 项与 Storage 11 项通过，Runtime/Desktop 构建和 tokens 检查通过；定向 ESLint 无 error，既有 Hook 提示保留。
- 实际 ChatView 的 2000 条合成目录初始只读 50 条正文，正文约 339–360 ms、完整目录约 605–614 ms、GC 后约 13.7MiB；这是新按需读取口径，不与此前全量正文挂载直接比较。
- Edge 与独立 Electron 沙箱验证 50→100 条跨页、真实缺口、代码实例/展开/横向 240px 保持、滚轮取消和 820px 窗口。10000 条目录仍约 5 秒，刻度密度与长期挂载仍待优化；真实库与用户窗口尚未切换。
- 详细读取基准、边界与证据见审查报告第 16 节。本批无新增依赖或迁移，不涉及用户历史物理清理。

## 2026-09-05：方案优化第六批——生产构建、按需页面与高亮收敛

### Changed

- 生产/开发/QA 使用显式模式和独立目录；生产 JS/CSS 压缩、不携带 sourcemap 或视觉入口，生成模块体积清单并实施首屏/总量预算。发布副本排除重复 Renderer 模块和 QA fixture，预检查拒绝非生产 manifest。
- 设置、工作台和六个次级页面使用本地 ESM 按需模块；加载状态和失败边界局限于页面。真实 Electron 验证发现失败模块缓存会令普通重试失效，现以生成的本地映射和跨重挂载递增地址恢复，不刷新主窗口。
- HTML/图表源码和代码/差异行共用限定语言注册表；未知语言与大于 100,000 字符的输入保留转义全文，不执行全语言自动识别。

### Verification

- 生产首屏 JS 静态依赖闭包 1,866,411 bytes（gzip 合计约 599KB），全部 Shell JS 2,684,829 bytes；同源码开发构建对应约 4.70/6.08MB。预算为 2.10/3.00MB，外置 vendor、CSS 和字体另计。
- Electron 沙箱中验证生产查询参数不启用 QA、设置/智能体首次进入才加载、模块缺失与恢复及失败重开后的再次重试、820×740 无横向溢出；开发/QA 页面独立可用。
- 同源码真实 ChatView、500/2000 条合成消息各 3 次：生产初屏约 0.74–0.87/2.02–2.08 秒，GC 后堆约 20/61MiB；2000 条仍有约 1.46–1.47 秒首次长任务，不宣称初屏优化已经结束。
- Desktop 全量 238 files / 1974 tests（2 Worker）、完整构建、20 项发布/QA 脚本测试通过；定向生产 ESLint 0 error / 2 个既有 ShellApp 依赖提示。详情与测量边界见审查报告第 15 节；实际库与用户窗口尚未切换。

## 2026-09-05：方案优化第五批——增量导航几何与长会话渲染

### Changed

- 导航缓存消息位置，行高增量通过前缀和树更新，当前轮次二分定位；滚动不再全量读取消息矩形，隐藏页面、prepend 和窗口变化正确重建。
- 历史可见性直接更新调度队列，不触发消息列表 React 渲染；缺省目录引用与导航刻度稳定，摘要不随高亮重复计算。
- 跳转仅展开目标行布局，等待连续稳定帧并有修正上限；手动操作取消跳转，轨道滚轮释放追底。现有消息保持挂载，阅读状态不交给新虚拟滚动。

### Verification

- 最终源码各做 3 次采样，500/2000 条实际 ChatView 隔离样本的最大跳转帧间隔分别约 11–16/35–46ms（前批约 230/1004ms），每组 72 帧均未超过 50ms，滚动消息矩形读取为 0；初屏和堆成本仍记录，不宣称整体性能验收已完成。
- 80 条成对问答与 80 行代码跨导航/调整窗口后，展开与 scrollTop=140、scrollLeft=220 保持；左侧位置、当前轮次、手动取消和窄窗口验证通过。详细证据与剩余范围见审查报告第 14 节。
- Desktop 全量 234 files / 1947 tests（2 Worker）及完整构建通过，生产定向 ESLint 0 error/既有 17 warning，差异检查通过。默认并发首次 Worker 异常退出日志保留，未把根因归为已证实的内存故障。

## 2026-09-05：方案优化第四批——历史过程请求调度与手动恢复

### Changed

- 历史过程请求从 ChatView 提取到共享 3 槽调度器，跨多栏/keyed 重建保留真实在途额度，活动与可见轮次优先，其余按新到旧补读。
- 连接/Worker/繁忙错误有限重试，总共最多 3 次；错误时正文保留，提供单轮手动读取恢复，不重跑历史任务。实时/terminal 竞态不会让旧读取覆盖新过程。
- 新增实际 ChatView 的 500/2000 消息基线；确认请求受控后仍存在大 DOM 与导航/渲染成本，继续按完整方案治理。

### Verification

- 新增 loader 23 项与实际 ChatView 4 项行为测试；Edge 隔离页面验证 60 轮可见优先、keyed 切换共享额度、6 轮最多 18 次尝试、手动恢复和两种窗口宽度。
- Desktop 全量 231 files / 1930 tests、完整构建通过；生产定向 ESLint 0 error，ChatView 保留原有 17 个依赖提示，差异检查通过。
- 隔离页面与真实内核/业务窗口验证分开；未迁移或清理实际用户库。详细口径、证据和剩余工作见审查报告第 13 节，整个优化目标仍在实施。

## 2026-09-05：方案优化第三批——异步冷恢复与有界执行过程读取

### Changed

- 原生任务快照捕获使用只读事务，安装使用 source-cursor 校验的短事务；生产请求冷读交给只读 Worker，归约期间不占用 Runtime 主线程或长持写锁。
- 冷筛选 SQL 一次提取工具元数据，关联原生请求/结果；去除重复 run 快照，避开无关工具大输出。
- 执行过程使用精简事件投影和 event-version 缓存，限制 32 个 Run / 16 MiB；历史 Worker 队列、超时、空闲释放和关闭取消均有边界。
- 可选任务快照缓存写失败保留源读取结果，并限频输出诊断。Runtime 停止时关闭 reader，真实 task/run 和 sidecar 合同保持不变。

### Verification

- Storage 45 files / 456 tests、Runtime 180 files / 1341 tests 全绿；构建、定向生产 ESLint 和差异检查通过。
- 完整 83 事件 Run 的隔离重放：旧读入/投影 318–373 ms；新 Worker 冷过程约 102 ms，任务冷恢复含启动约 288 ms，暖读取均小于 0.25 ms；过程内容与旧读取严格一致，冷读期间主线程定时器持续响应。
- 未修改用户实际数据库或重启加载新业务代码。性能口径、来源、剩余范围与最终窗口步骤见审查报告第 12 节；当前仍非整个方案完成。

## 2026-09-05：方案优化第二批——审批一致性、代码阅读与可用设置

### Changed

- 审批决定与永久应用授权采用同一事务，会话授权提交后激活；失败保留 waiter 和原审批 ID，重试不泄漏授权或执行工具。
- 代码块阅读状态按当前消息和源文本位置保存，跨流式分段冻结、二级章节转换和生成完成恢复展开、暂停跟随及横纵滚动位置；替换内容清空旧状态。
- 设置目录与搜索共用可用性规则，移除未接通页面/云同步开关；保留插件和快捷键旧链接的有效目的地、真实数据管理功能。
- 同步四项旧 Desktop 测试及设置导航检查，补充 Think 底部跟随与手动上滚分支。

### Verification

- Desktop 229 files / 1903 tests、Runtime 179 files / 1332 tests 全绿；两包构建、定向生产 ESLint 和差异检查通过。
- 本地 Edge 的隔离真实组件页面验证：代码完成前后展开状态及横纵位置不变、未实现设置不进入搜索；1280×900 与 820×740 无相关控制台错误或设置页横向溢出。
- 实际数据库和运行中窗口尚未切换；完整方案仍在推进。证据、后续范围和页面步骤见 `docs/reviews/2026-09-05-codebase-audit.md` 第 11 节。

## 2026-09-05：方案优化第一批——审批查询与原生任务持久快照

### Changed

- 审批恢复改为 thread/run、approvalId 专用索引查询，移除在线审批查询的全量历史 hydration；孤儿审批查询时持久失效，失效落盘失败不确认成功。
- 新增原生任务派生快照与 task 游标索引；快照随事件原子更新，checkpoint 失败同步回滚；`conversation.listMessages` 重启后直接读取快照，不要求内核维护第二套任务。
- 提取审批归约和任务快照模块。文件变更预览统一限长，但保留完整 content 供差异展示。
- 同步迁移、审批读取与 Claude 原生任务指令回归，保留作用域隔离和真实工具合同。

### Verification

- Storage 45 files / 452 tests、Runtime 178 files / 1326 tests 全绿；两个包构建、本批生产源文件 ESLint 与空白差异检查通过。
- 历史 Task 的只读样本在隔离内存数据库重放：首次快照约 749 ms，暖读取 0.018–0.072 ms；首次冷读取和完整窗口耗时仍需进一步优化/实测。
- 新迁移尚未应用到用户实际数据库；最终窗口重启、真实内核矩阵和其余五个方向仍在完整目标内。细节及页面验收步骤见 `docs/reviews/2026-09-05-codebase-audit.md` 第 10 节。

## 2026-09-05：代码块、工具结果与文件差异展示升级

### Changed

- 按已确认的 beUI 方向适配三类内容展示，保留主标签栏、原生内核事件与任务持久化链路；未安装 Motion、Shiki 或另一套 AI SDK。
- 普通代码块统一文件/语言栏、状态、行号、高亮、复制与长代码展开；实时输出支持暂停跟随和返回末尾。Markdown token 更新保持代码节点与内部阅读位置，特殊图表/HTML/设计稿渲染分支保留。
- 工具参数和 JSON 结果保留结构化字段并支持复制原文；普通结果使用代码面板。实时结果默认展开、成功收起、失败保持可见，用户手动选择优先，不添加无后端支持的重试按钮。
- 文件差异保留真实快照与现有 LCS 算法，补双行号、源码高亮、增删统计和修改后内容复制。截断/缺失/超大快照明确降级，修正空文件产生虚构删除行的问题。

### Verification

- 7 个相关测试文件共 142 项通过，覆盖 Markdown 特殊内容/资源链接、代码复制与流式节点稳定性、工具披露状态、文件差异、RightDock 和 WorkspaceWorkbench；桌面 TypeScript / preload / renderer 构建、设计 token 检查通过。
- 内置浏览器以真实组件和生产 CSS 验证 840px / 340px 内容列、明暗及壁纸主题：代码回看后连续追加仍保持 scrollTop=0，窄列无外溢，工具成功收起/失败展开、双行号及截断快照提示符合预期，控制台无警告/错误。浏览器使用独立测试数据，未连接真实内核。
- 桌面全量首轮为 1886 通过、5 未通过；其中浏览器桥接超时用例按项目 15 秒超时设置复测通过。其余 4 项记录为本轮范围外：`ChatView.turn-skill.test.tsx` 的无 taskId 清单预期、`ChatView.scroll-anchor.test.tsx` 的 Think 文本唯一性、`tests/run-process-wiring.test.ts` 的旧源码字串匹配、`tests/phase3-accessibility.test.ts` 的截图路由静态断言。本轮未扩展修改这些测试或对应行为。
- 手工验收：重启桌面加载构建 → 让模型输出多行代码 → 上滚后观察追加；展开工具参数/结果并复制 → 检查失败仍可见；打开本轮文件变更 → 核对新旧行号、高亮、换行和真实文件入口。本轮未重启桌面窗口或 Runtime。

## 2026-09-05：左侧问答导航与轻量预览

### Changed

- 消息导航保留左侧位置和按问答轮次居中的刻度，采用中性细线与更长的当前项，移除普通/壁纸主题的光晕。命中轨道扩至 32px，按最近刻度连续预览和点击，外围空白不抢占消息操作。
- 预览改为「问题主标题 + 回答次级摘要 + 弱化时间」，各最多两行，宽度受聊天列约束；执行中显示简短状态，移除 commentary/运行信息的重复堆叠。键盘焦点关联预览，Escape 可关闭，减少动态效果设置生效。
- 每轮携带首条提问的消息锚点，点击优先回到问题；分页边界缺少问题时回退到回答。保留长消息预热/位置校正、分页和暂停贴底机制，修正长回答尚未读完就提前高亮下一轮的问题。

### Verification

- 导航组件、ChatView 历史加载/用量与消息窗口 4 个测试文件共 47 项通过；桌面 TypeScript / preload / renderer 构建、设计 token 检查及 `git diff --check` 通过。
- 浏览器使用真实导航组件与生产 CSS 的独立页面验证：860px / 320px 聊天列、明暗/壁纸主题、6 / 120 轮。32px 透明区域可点击，问题锚点误差小于 1px；键盘预览不滚动、Escape 关闭；预览标题和摘要各两行，窄列及轨道底部未溢出，执行中仅显示简短状态，控制台无警告/错误。
- 交互夹具中点击历史后追加流式内容，scrollTop 保持 454；未连接真实内核，本轮未重启桌面窗口。手工验收入口：打开多轮会话 → 左侧悬停/点击 → 检查问题定位；生成期间回看历史 → 确认不自动拉回底部。

## 2026-09-05：原生内核任务回显与重启恢复

### Fixed

- Claude Code 使用原生 TaskCreate / TaskUpdate / TaskList / TaskGet，兼容 TodoWrite；Codex 使用原生 update_plan 和 turn/plan/updated。两个外部内核不再注入宿主 task-board MCP 工具或要求创建第二份平台清单。
- Claude SDK 保留原生结构化工具结果、任务编号及描述；成功结果更新回显，失败或尚未执行的原生任务不冒充成功。支持删除、清空、TaskList 摘要保留已知描述，以及 TaskGet 补齐说明。Codex 多行步骤保留标题和说明。
- conversation.listMessages 返回当前会话的持久任务投影，使用 task 索引只读取任务工具及运行生命周期事件；桌面将此快照与后续实时事件合并。保留全局活动游标，不重放全库事件，修复重启后消息仍在而任务面板丢失。
- 内置模型模式补齐生产 Runtime 的 SqliteTaskPlanStore 注入。Composer 回显单项原生任务；Claude 原生任务可跨续聊保持，Codex/旧平台计划仍按轮次更新，切换会话不串清单。旧记录缺少说明时继续保留原标题，不补造内容。

### Verification

- 覆盖原生任务创建/更新/删除/读取、SDK 结构化结果、Codex 计划、MCP 目录与系统指令、真实 Runtime 关闭重开、空活动事件流下的 ChatView 恢复和会话隔离。
- 用户原会话只读验证：筛选得到 26 条相关事件，恢复原五项清单及 1/5 进度；恢复快照 368 字节，不重置活动游标、不修改历史任务数据。
- 16 个定向测试文件、260 项测试通过，全量构建 11/11、设计 token 检查与 diff 空白检查通过。Playwright 连接真实 Electron 桌面（Browser plugin not available），确认原会话五项清单恢复、折叠/展开和页面重载后仍显示 1/5，重载无页面运行时错误。首次实窗检查发生旧守护实例重连超时，清理旧实例后重新验证通过。

## 2026-09-05：任务清单保留并展示具体执行说明

### Fixed

- `update_task_plan` 增加任务项 `description`，提示模型提供具体范围、目标文件/模块、检查项或产出；完整列表更新继续携带说明，而不是只生成含糊标题。
- TaskCreate / TaskUpdate / TaskList 的清单快照、运行事件投影和下一轮模型上下文保留已有说明；Codex 原生计划支持首行标题、后续行说明。旧单行清单保持兼容，不编造历史详情。
- Composer 任务项显示「标题 + 次级说明」，长标题、路径和多行说明自动换行；列表高度上限使用独立 token，在常规窗口完整展示五项双行清单，窄窗口/更长列表滚动查看。完成态保留说明，折叠/清除交互不变，仅说明发生更新也会重新展示清单。

### Verification

- Desktop / Runtime 共 8 个定向测试文件、138 项测试通过；覆盖清单执行、持久化快照、上下文回填、Codex 原生计划、事件投影、双行展示和清除后更新。全量构建 11/11 通过，设计 token 检查通过。
- Playwright 使用真实 Composer 组件、工具返回值及事件投影测试数据，验证 1000×800 深色和 390×760 浅色视口；五项说明完整展示，长路径换行、状态更新、折叠/展开、清除及说明更新通过，页面无运行时错误。开发版已重启，Runtime、数据库与桌面握手就绪。

## 2026-09-05：执行过程活动条、任务清单、贴底滚动与对话浏览器

### Fixed

- 底部活动条改为显示最新 Think 行。关掉 Think 开关只隐藏时间线行，等待模型时不再只写「思考中」或「等待模型响应」。
- `goal_manage` 只在用户开启 Goal 模式后进入工具目录。任务清单继续走 `update_task_plan`，并出现在 Composer 任务面板；两者不是同一回事。
- 流式过程增长会跟随信息流贴底。用户上滚仍会松开跟随。
- Codex 未分类正文现在作为过程车道里的流式说明输出，不再等工具边界后整段弹出。
- 对话里的浏览器工具改在右侧工作台打开，主区域对话保持可见。
- 绑定文件夹若不是 git 仓库，系统提示会明确告诉模型不要跑 git，避免把 `not a git repository` 刷成一串红失败。

### Verification

- Desktop `process-activity`、`InlineProcessFlow`、`ChatView.messageToChat`、`message-window`、`workspace-workbench`、`ShellApp`；Runtime `registry`、`platform-tools`、`workspace-context` 定向回归。

## 2026-09-05：已更改文件默认最多露出 4 条

### Changed

- 「已更改 N 个文件」默认只列出前 4 条。超过 4 条时底部出现「还有 N 个文件」，点击后以高度过渡展开其余路径，可再收起。

### Verification

- Desktop `ExecutionProcessBlock` 定向回归。

## 2026-09-05：命令段等下一段正文再收起，已更改文件跟在最终回答后

### Changed

- 连续命令段在工具仍在跑、或刚跑完但还没进入下一段思考/说明/最终回答时保持展开；下一段正文开始后才自动收起。
- 「已更改 N 个文件」从执行过程面板挪到最终回答正下方，不再出现在正文前面。

### Verification

- Desktop `InlineProcessFlow`、`ChatView.usage` 定向回归。

## 2026-09-05：连续工具段在思考中间就地折叠

### Changed

- 执行过程不再把整轮所有工具收进一条「运行了 N 个命令」总摘要。思考、说明和状态仍按原文位置穿插；只有中间连续的工具行（至少 2 条）合并成可点开的动作摘要。
- 单独一条工具仍直接显示。某一段里有正在运行的工具时该段默认展开，完成后默认收起。

### Verification

- Desktop `process-activity`、`InlineProcessFlow` 定向回归。

## 2026-09-04：执行过程保留完整思考、去掉截断提示

### Fixed

- 长 Run 不再把较早的 Think 从内存窗口和持久化时间线里丢掉。命令可以按窗口淘汰，思考和说明正文始终保留，所以过程不会在后半段只剩「命令执行」。
- 执行过程不再显示 `N earlier process segments truncated for durable storage`。展开过程时会把已加载的完整片段和当前时间线合并，而不是用首页把最近的思考盖掉。
- 完整时间线分页会先压缩超大的命令输出，避免 1MB 帧上限把「加载完整思考」整页打掉。

### Verification

- Runtime `assistant-timeline-blocks`、`demo-run.timeline-bound`；Desktop `ChatView.messageToChat`、`ChatView.usage` 定向回归。

## 2026-09-04：执行过程 Think 开关与中文动作摘要

### Added

- 执行过程标题行右侧增加独立 Think 开关。关掉后时间线不再显示 Think 行，设置里的「显示思考过程」与该开关共用偏好。
- 执行过程增加中文动作摘要折叠行，例如「探索了 3 个文件，运行了 7 个命令」。收起后隐藏工具行；流式时默认展开，完成后默认收起。

### Verification

- Desktop `InlineProcessFlow`、`process-activity`、`SettingsPage`、`ui-preferences` 定向回归。

## 2026-09-04：工作台加号跟随标签并支持新建对话

### Fixed

- 主区域和工作台的 `+` 不再钉在标签栏最右侧，改为紧跟当前最后一个标签。
- 点击主区域时只显示主标签栏 `+`；点击右侧或底部工作台时只显示对应工作台的 `+`。
- 右侧/底部工作台的 `+` 菜单在「工作区文件」下方提供「新建对话」。新对话作为工作台标签打开，不再落到左边主窗格。

### Verification

- Desktop `ConversationTabs`、`WorkspaceWorkbench` 定向回归。

## 2026-09-04：对话文件树与工作台浏览器保活

### Changed

- 工作区「对话文件」改为与「所有文件」相同的嵌套文件树：按项目相对路径展开文件夹，不再把完整目录路径摊成一组一组的扁列表。

### Fixed

- 工作台「浏览器」标签此前写死地球图标。对话页标签栏已经接过页面 favicon，但 NewMax 工作台没有接到同一套 `onPageMeta`，所以站点 logo 一直不出现。
- 切换 README / 对话等其它标签时，工作台会把 `BrowserPanel` 卸掉。空标签里导航过的地址又没有写回 tab.url，回来就只剩空白页。现在浏览器 guest 保持挂载，并把当前 URL 写回工作台标签。

### Verification

- Desktop `workspace-file-tree`、`RightDock`、`workspace-workbench`、`WorkspaceWorkbench` 定向回归。
