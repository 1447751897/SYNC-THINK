...
| #   | 决策         | 定案                                                                                                                        |
| --- | ------------ | --------------------------------------------------------------------------------------------------------------------------- |
| Q6  | 崩溃恢复     | **登录自启 + 桌面应用兜底**：计划任务优先，系统策略拒绝时回退 HKCU Run；应用启动时检测守护进程不在则拉起。无需 Windows 服务 |
| Q8  | 开机自启默认 | **默认注册，设置里可关；显式关闭偏好持久保存**                                                                              |
...
| T3 自启 + 兜底     | #5 ✅  | `c391e55` | `autostart.ts`（schtasks / HKCU Run）+ daemon CLI 入口 + 桌面 ensureDaemonProcess                               |
...
- 设置页自启开关与**默认注册**策略已实现（T11）：显式 opt-out 持久化；计划任务注册被策略拒绝时回退到当前用户 Run 登录启动项；桌面仍保留 `ensureDaemonProcess` 兜底。
