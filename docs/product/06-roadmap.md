## 2026-09-07：官网、可嵌入演示与云端账号

用户新增自有服务器和登录需求，已确认自托管邮箱账号方案。云端身份作为独立产品阶段启动，原本地工作台与工具权限边界继续适用。

- 本期：品牌介绍页、下载/文档入口、独立 iframe 交互演示、注册/验证/登录/找回/退出/账号页与部署配置。
- 上线待办：提供目标域名与服务器环境、配置 HTTPS 和 SMTP、执行真实邮件与备份恢复验收；详见 `docs/engineering/17-cloud-account-launch.md`。
- 下一阶段：桌面登录和设备配对/撤销、有限对象同步及冲突处理。
- 后续：具备租户隔离与执行隔离后的云端 Agent、团队角色、配额、队列和审计。账号上线不自动启用云同步、钱包或团队入口。
