...
| 角色 / 主体 | 可执行操作 | 不可执行操作 |
| --- | --- | --- |
| 用户（真人） | 配置 Provider/Agent/Skill/策略；选择完全访问；批准计划与受保护动作；暂停/取消 Run；导出诊断（脱敏） | 突破智能体能力上限或执行当前不可用能力 |
| 审批 Agent（可选） | 在策略范围内评估受保护动作 | 在请求批准、替我审批或自定义模式下批准必须由本人确认的敏感操作 |
...
- 请求批准：读取类操作自动执行；写入、命令和对外操作暂停并请求用户批准。
- 替我审批：受保护操作由指定审批智能体评估；不能委托的敏感操作回退本人确认。
- 完全访问：所有当前可执行操作直接运行，不弹任何审批；上述七类敏感操作也直接运行，但仍保留审计记录。
- 自定义：按工具 / 目录 / 命令 / 站点 / 动作配置规则。

Run 级策略优先于任务级，任务级策略优先于更宽泛的 Agent / 项目 / 用户默认。`完全访问` 一旦成为最终有效模式，遗留单项 request/delegate 规则不得重新触发审批；能力上限、授权目录和当前是否具备该工具仍然有效。
...
6. **默认安全**：工具访问受能力和资源范围约束；完全访问显式全自动，其他模式对敏感操作保持门禁。
