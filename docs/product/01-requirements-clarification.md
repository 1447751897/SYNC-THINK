...
1. 当前模型能力按 `supported / unsupported / unknown` 三态处理：`supported` 与 `unknown` 都先直接尝试原生图片输入；只有明确 `unsupported` 时才进入视觉 Fallback。环境性探测失败（鉴权、限流、超时、网络）保持 `unknown`，不能当作不支持视觉。
2. 非视觉模型收到图片附件时，若“图片识别 Fallback”开关开启、已选择模型、所属 Provider 启用且所选模型已确认支持视觉，由 Runtime 在主模型调用前生成图片描述并注入本轮上下文。
3. 视觉 Fallback 未启用、配置无效或当前模型视觉能力为 `unknown` 时，按 NewMax 规则保留原图并尝试当前模型的原生图片输入；Windows OCR 仅作为模型主动读取工作区图片文字的工具。
4. 已确认文本模型的视觉 Fallback 调用失败或未返回有效描述时，原图不得发送给该文本模型；本轮上下文和界面必须明确说明图片转写失败，避免模型猜测图片内容。
5. `describe_image` 与 `ocr_image` 继续作为工作区已有图片的主动读取工具，不承担聊天附件的自动路由职责。
6. 附件处理结果至少区分 `forwarded / described / failed`（历史 `ocr` 事件保留兼容展示），并进入 `run.started` 与 append response，供诊断和用户提示使用。
