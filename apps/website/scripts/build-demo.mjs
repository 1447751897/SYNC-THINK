...
const desktopRoot = path.resolve(websiteRoot, '../desktop');
...
export async function buildWebsiteDemo(output) {
  ...
  const result = await build({
    ...
    entryPoints: { 'chat-app': path.join(websiteRoot, 'src/demo/WebsiteChatDemo.tsx') },
    outdir: assets,
    bundle: true,
    platform: 'browser',
    format: 'esm',
    ...
    plugins: [
      {
        name: 'desktop-demo-surface',
        setup(builder) {
          builder.onResolve({ filter: /^@sync-think\/desktop-demo-surface$/ }, () => ({
            path: path.join(shell, 'website-demo-surface.ts'),
          }));
        },
      },
      {
  ...
  await cp(path.join(websiteRoot, 'src/demo/website-demo.css'), path.join(assets, 'chat-app.css'));
  await writeFile(
    ...
    JSON.stringify(
      {
        ...
        components: inputs
          .filter(
            (input) =>
              (input.includes('renderer/shell/') || input.includes('src/demo/')) &&
              input.endsWith('.tsx'),
          )
