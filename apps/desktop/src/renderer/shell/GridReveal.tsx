/**
 * RareUI Grid Reveal (https://www.rareui.com/components/gridreveal) ported
 * without the `motion` dependency. Four opening cells keep splitting the
 * largest remaining cell until the photo arrives, then the grid yields to
 * the image. Caption shimmer is CSS.
 */
...
function orderByDetail(branches: Cell[], openedBefore: number) {
  const pending = branches.filter((cell) => cell.splitAt > openedBefore);
  if (pending.length < 2) return;
  const slots = pending.map((cell) => cell.splitAt).sort((a, b) => a - b);
  const queue = pending.filter((cell) => !cell.parent || cell.parent.splitAt <= openedBefore);
  let next = 0;
...
export function GridReveal({
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      let target: number;
      if (ready) target = 1;
      else if (paced) target = selfPaced(elapsed * 1000, durationRef.current);
      else target = Math.min(clamp01(progressRef.current as number), HOLD);
  ...
  return (
    <div
      ...
      className={clsx('shell-grid-reveal', src && 'is-zoomable', className)}
      style={{ aspectRatio: ratio, cursor: src ? 'zoom-in' : undefined, ...style }}
      {...props}
      onClick={src ? () => setLightboxOpen(true) : props.onClick}
    >
      <canvas
        ref={canvasRef}
        className="shell-grid-reveal__canvas"
        {...(alt ? { role: 'img', 'aria-label': alt } : { 'aria-hidden': true })}
      />
      {caption ? (
        <div
          className={clsx('shell-grid-reveal__caption', finished && 'is-done')}
          data-testid="grid-reveal-caption"
        >
          <span className="shell-grid-reveal__caption-text">{caption}</span>
        </div>
      ) : null}
      <ImageLightbox
