...
export function AbilitiesPage(props: {
  ...
  const [skillCategory, setSkillCategory] =
    useState<(typeof SKILL_CATEGORIES)[number]>('全部');
  ...
  const [registerMcpItem, setRegisterMcpItem] = useState<McpMarketItem | null>();
  ...
  const loadCatalog = useCallback(async () => {
    ...
    try {
      ...
      if (requestId !== loadRequestRef.current) return;
      setLoadError(cause instanceof Error ? cause.message : '能力库加载失败');
    } finally {
  ...
  const selectedSkill = skills.find(
    (skill) => skill.skillVersionId === detailSkillVersionId,
  );
  ...
  const installMarketSkill = useCallback(
    async (item: SkillMarketItem) => {
      ...
      try {
        ...
        setError(cause instanceof Error ? cause.message : '安装 Skill 失败');
      } finally {
        setBusyId(undefined);
      }
    },
  ...
  const openSkillEditor = useCallback(async (skill?: SkillVersionSummary) => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : '读取 Skill 失败');
    } finally {
      setBusyId(undefined);
    }
  }, []);
  ...
  const deleteSkill = useCallback(
    async (skill: SkillVersionSummary) => {
      ...
      if (!window.confirm(`确定卸载“${skill.name}”v${skill.version}？`)) return;
      ...
      try {
        ...
        setMessage(`已卸载：${skill.name} v${skill.version}`);
        await refreshAfterMutation();
      } catch (cause) {
        setError(cause instanceof Error ? cause.message : '卸载 Skill 失败');
      } finally {
        setBusyId(undefined);
      }
    },
  ...
  const setSkillGlobalEnabled = useCallback(
    async (skill: SkillVersionSummary, enabled: boolean) => {
      ...
      try {
        ...
        setError(cause instanceof Error ? cause.message : '更新 Skill 全局状态失败');
      } finally {
        setBusyId(undefined);
      }
    },
  ...
  const setMcpGlobalEnabled = useCallback(
    async (server: McpServerSummary, enabled: boolean) => {
      ...
      try {
        ...
        setError(cause instanceof Error ? cause.message : '更新 MCP 全局状态失败');
      } finally {
        setBusyId(undefined);
      }
    },
  ...
  const setWorkspaceActive = useCallback(
    async (
      ...
      try {
        ...
        setError(cause instanceof Error ? cause.message : '更新工作区激活状态失败');
      } finally {
        setBusyId(undefined);
      }
    },
  ...
  const registerMcp = useCallback(
    async (payload: {
      ...
      setError(undefined);
      try {
        const result = await api.registerMcpServer(payload);
        setRegisterMcpItem(undefined);
        setMessage(result.updated ? `已更新 MCP：${result.server.name}` : `已注册 MCP：${result.server.name}`);
        setDetailMcpServerId(result.server.mcpServerId);
        setTab('mine');
        await refreshAfterMutation();
      } catch (cause) {
        setError(cause instanceof Error ? cause.message : '注册 MCP 失败');
      } finally {
        setBusyId(undefined);
      }
    },
  ...
  const refreshMcpTools = useCallback(
    async (server: McpServerSummary) => {
      ...
      try {
        ...
        setError(cause instanceof Error ? cause.message : '刷新 MCP 工具失败');
      } finally {
        setBusyId(undefined);
      }
    },
  ...
  const previewOrganize = useCallback(async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : '生成整理报告失败');
    } finally {
      setBusyId(undefined);
    }
    ...
    <main className="capability-center" data-testid="abilities-page">
      ...
      {section === 'skills' ? (
        ...
        <McpSurface
          ...
          onWorkspaceActive={(server, active) =>
            void setWorkspaceActive('mcp', server.mcpServerId, active)
          }
      ...
      <SkillDetailDrawer
        ...
        governance={selectedSkill ? skillGovernanceMap.get(selectedSkill.skillVersionId) : undefined}
        workspaceName={workspaceName}
          ...
          <div className="capability-governance-list" role="table" aria-label="Skill 治理列表">
            ...
            {visibleFamilies.map((family) => {
              ...
              return (
                <article
                  key={family.skillId}
                  className="capability-governance-row"
                  role="row"
                >
                  <button
                    type="button"
...
function McpSurface(props: {
  ...
  onWorkspaceActive(server: McpServerSummary, active: boolean): void;
  ...
  const visibleServers = props.servers.filter((server) => {
    ...
    const statusMatches =
      props.statusFilter === 'all' ||
      (props.statusFilter === 'active' && usage.callCount > 0) ||
      (props.statusFilter === 'enabled' && server.enabled !== false && workspaceActive) ||
      (props.statusFilter === 'inactive' && !workspaceActive) ||
      (props.statusFilter === 'unused' && usage.callCount === 0) ||
      (props.statusFilter === 'problem' &&
    ...
    <section className="capability-center__content capability-center__content--mine">
      ...
      <div className="capability-list-shell">
        {props.loading ? (
          ...
          <div className="capability-governance-list" role="table" aria-label="MCP 治理列表">
            <div className="capability-governance-list__header" role="row">
              <span>MCP 服务</span>
              <span>连接</span>
              <span>工作区激活</span>
              ...
              return (
                ...
                >
                  ...
                  <WorkspaceActivationControl
                    active={workspaceActive}
                    workspaceName={props.workspaceName}
                    testId={`mcp-workspace-activation-${server.mcpServerId}`}
                    busy={props.busyId === `workspace:mcp:${server.mcpServerId}`}
                    disabled={Boolean(props.busyId) || server.enabled === false}
                    onChange={(active) => props.onWorkspaceActive(server, active)}
                  />
                  <strong className="capability-governance-row__metric">
                    {usage.callCount.toLocaleString('zh-CN')}
                  </strong>
  ...
  return (
    <div className="capability-stats">
      ...
      >
        <div>
          <span>常驻上下文占用</span>
          <strong>{contextPercent >= 100 ? `≈ 超限 ${(props.contextTokens / CONTEXT_BUDGET_TOKENS).toFixed(1)}x` : `${contextPercent}%`}</strong>
        </div>
      ...
      {props.showSource ? (
        <label className="capability-source-select">
          ...
          <select
            ...
            onChange={(event) =>
              props.onSourceFilterChange(event.target.value as SourceFilter)
            }
          >
...
function WorkspaceActivationControl(props: {
  ...
  return (
    ...
    >
      <span>
        {props.busy ? <Loader2 className="animate-spin" size={10} /> : props.active ? <Check size={10} /> : null}
      </span>
...
function StatCard(props: {
  ...
  return (
    <article className={props.tone === 'warning' ? 'capability-stat is-warning' : 'capability-stat'}>
      <div>
...
function EmptyState(props: {
  ...
  return (
    <div
      className={
        props.compact ? 'capability-empty-state is-compact' : 'capability-empty-state'
      }
    >
      <span>
        <Boxes size={22} />
...
function SkillDetailDrawer(props: {
  ...
  const issue = Boolean(
    skill &&
      ((usage?.problemCount ?? 0) > 0 || skill.hasScripts || skill.warnings.length > 0),
  );
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="capability-drawer">
                ...
                <section className="capability-detail-section">
                  ...
                  <div className="capability-effect-chain">
                    <div className="capability-effect-chain__path">
                      ...
                      <span
                        className={
                          props.governance?.workspaceActive ? 'is-done' : undefined
                        }
                      >
                        <Check size={11} />
                        工作区激活
          ...
          <footer className="capability-drawer__footer">
            {skill ? (
              <>
                <button type="button" className="capability-button is-quiet" onClick={props.onGoToAgents}>
                  前往 Agent
                </button>
                <button
                  type="button"
              ...
              <>
                ...
                <section className="capability-detail-section">
                  ...
                  <dl className="capability-detail-list">
                    ...
                    <div>
                      <dt>可信状态</dt>
                      <dd>{server.trusted ? '可信来源' : '未标记可信'}</dd>
                    </div>
                    <div>
                      <dt>超时</dt>
                    ...
                    <div className="capability-mcp-tool-list">
                      {server.tools.map((tool) => (
                        <div key={tool.name}>
                          ...
                          <span>{tool.description || '未提供工具说明'}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
...
function DetailMetric(props: {
  ...
  return (
    <div className={props.tone ? `capability-detail-metric is-${props.tone}` : 'capability-detail-metric'}>
      <span>{props.label}</span>
      <strong>{props.value}</strong>
    </div>
  );
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="capability-dialog capability-dialog--editor">
          <header className="capability-dialog__header">
            ...
            <Dialog.Close asChild>
              <button type="button" className="capability-icon-button" aria-label="关闭 Skill 编辑">
                <X size={17} />
              </button>
            </Dialog.Close>
          </header>
            ...
            <div className="capability-editor-toolbar">
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  if (file) {
                    void file.text().then((text) => handleSourceChange(text.replace(/^\uFEFF/, '')));
                  }
                  event.target.value = '';
                }}
              />
          ...
          <footer className="capability-dialog__footer">
            ...
            <button
              ...
              className="capability-button capability-button--primary"
              disabled={props.saving || !source.trim() || !name.trim()}
...
function SkillSourceDialog(props: {
  ...
  useEffect(() => {
    ...
    void api
      ...
      .catch((cause: unknown) => {
        if (!cancelled) {
          setError(cause instanceof Error ? cause.message : '读取 SKILL.md 失败');
        }
      })
      .finally(() => {
...
function SkillPublishDialog(props: {
  ...
  const selectedSkill = props.families
    .flatMap((family) => family.versions)
    .find((skill) => skill.skillVersionId === props.skillVersionId);
  ...
  useEffect(() => {
    const api = runtimeBridge();
    if (!props.open || !selectedSkill || !api?.getSkill) return;
    let cancelled = false;
    setLoading(true);
    setDraft(undefined);
    void api
      ...
      .catch((cause: unknown) => {
        if (!cancelled) {
          props.onError(cause instanceof Error ? cause.message : '读取发布内容失败');
        }
      })
      .finally(() => {
        if (!cancelled) setLoading(false);
  ...
  }, [props.open, props.skillVersionId]);
  ...
  const saveDraft = async (): Promise<SkillPublishDraftSummary | undefined> => {
    ...
    try {
      ...
      props.onError(cause instanceof Error ? cause.message : '保存发布草稿失败');
      return undefined;
    } finally {
      setSaving(false);
    }
  ...
  const submitDraft = async () => {
    ...
    try {
      ...
      props.onError(cause instanceof Error ? cause.message : '提交发布草稿失败');
    } finally {
      setSaving(false);
    }
  };
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="capability-dialog capability-dialog--publish">
          <header className="capability-dialog__header">
            ...
            <Dialog.Close asChild>
              <button type="button" className="capability-icon-button" aria-label="关闭发布 Skill">
                <X size={17} />
              </button>
            </Dialog.Close>
          </header>
              ...
              <>
                <label className="capability-form-field">
                  ...
                  <select
                    ...
                    onChange={(event) => props.onSkillVersionChange(event.target.value || undefined)}
                  >
...
function McpRegisterDialog(props: {
  item: McpMarketItem | null | undefined;
  open: boolean;
  saving: boolean;
  error?: string;
  onOpenChange(open: boolean): void;
  ...
  useEffect(() => {
    ...
    setTransport(props.item?.transport ?? 'local-stdio');
    setEndpoint(props.item?.endpoint ?? '');
    setNotes(props.item?.notes ?? '');
    setTrusted(Boolean(props.item));
    ...
    <Dialog.Root open={props.open} onOpenChange={props.onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="capability-dialog capability-dialog--register">
          <header className="capability-dialog__header">
            <div>
              <Dialog.Title>注册 MCP</Dialog.Title>
              <Dialog.Description>保存服务配置并同步到 MCP 发现目录。</Dialog.Description>
            </div>
            <Dialog.Close asChild>
              <button type="button" className="capability-icon-button" aria-label="关闭 MCP 注册">
                <X size={17} />
              </button>
            </Dialog.Close>
          </header>
            ...
            <div className="capability-publish-grid">
              <label className="capability-form-field">
                ...
                <select
                  value={transport}
                  onChange={(event) =>
                    setTransport(event.target.value as 'local-stdio' | 'remote-http')
                  }
                >
              ...
              <label className="capability-form-field">
                <span>{transport === 'local-stdio' ? '启动命令' : '远程 Endpoint'}</span>
                <input
                  value={endpoint}
                  spellCheck={false}
                  placeholder={
          ...
          <footer className="capability-dialog__footer">
            ...
            <button
              ...
              className="capability-button capability-button--primary"
              disabled={props.saving || !name.trim() || !endpoint.trim()}
              onClick={() =>
                props.onSubmit({
                  name: name.trim(),
                  transport,
