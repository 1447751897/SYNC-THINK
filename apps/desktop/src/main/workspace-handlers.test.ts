import { describe, expect, it, vi } from 'vitest';
import { WORKSPACE_RUNTIME_IPC_CHANNELS } from '../runtime-bridge-contract.js';
import { registerWorkspaceHandlers, type WorkspaceHost } from './workspace-handlers.js';

function fixture() {
  const handlers = new Map<string, (event: string, value: unknown) => Promise<unknown>>();
  const order: string[] = [];
  const response = { workspaceId: 'workspace-1' };
  const request = vi.fn(async (command: string) => {
    order.push(`request:${command}`);
    return response;
  });
  const host = {
    handle: (channel: string, listener: (event: string, value: unknown) => Promise<unknown>) => {
      handlers.set(channel, listener);
    },
    assertSource: vi.fn(() => order.push('source')),
    ensureConnection: vi.fn(async () => {
      order.push('connect');
    }),
    requestWorkspace: request as WorkspaceHost<string>['requestWorkspace'],
  };
  registerWorkspaceHandlers(host);
  return { handlers, host, order, request, response };
}

describe('Workspace lifecycle IPC boundary', () => {
  it('registers the five workspace commands', () => {
    expect([...fixture().handlers.keys()]).toEqual(Object.values(WORKSPACE_RUNTIME_IPC_CHANNELS));
  });

  it.each([
    [
      WORKSPACE_RUNTIME_IPC_CHANNELS.create,
      { name: ' Atlas ', folderPath: ' D:/projects/atlas ' },
      'workspace.create',
      { name: 'Atlas', folderPath: 'D:/projects/atlas', allowedRoots: undefined },
    ],
    [
      WORKSPACE_RUNTIME_IPC_CHANNELS.bindFolder,
      { workspaceId: ' workspace-1 ', folderPath: ' D:/projects/atlas ' },
      'workspace.bindFolder',
      {
        workspaceId: 'workspace-1',
        folderPath: 'D:/projects/atlas',
        allowedRoots: undefined,
      },
    ],
    [WORKSPACE_RUNTIME_IPC_CHANNELS.list, null, 'workspace.list', {}],
    [
      WORKSPACE_RUNTIME_IPC_CHANNELS.update,
      { workspaceId: ' workspace-1 ', name: ' Atlas ', hidden: false },
      'workspace.update',
      {
        workspaceId: 'workspace-1',
        name: 'Atlas',
        folderPath: undefined,
        icon: undefined,
        sortOrder: undefined,
        hidden: false,
      },
    ],
    [
      WORKSPACE_RUNTIME_IPC_CHANNELS.delete,
      { workspaceId: ' workspace-1 ' },
      'workspace.delete',
      { workspaceId: 'workspace-1' },
    ],
  ])('forwards %s through its typed command', async (channel, value, command, payload) => {
    const { handlers, order, request, response } = fixture();
    await expect(handlers.get(channel)!('trusted', value)).resolves.toBe(response);
    expect(request).toHaveBeenCalledWith(command, payload);
    expect(order).toEqual(['source', 'connect', `request:${command}`]);
  });

  it('rejects untrusted senders before connection, parsing and transport', async () => {
    const { handlers, host, request } = fixture();
    host.assertSource.mockImplementation(() => {
      throw new Error('untrusted sender');
    });
    await expect(
      handlers.get(WORKSPACE_RUNTIME_IPC_CHANNELS.create)!('untrusted', null),
    ).rejects.toThrow('untrusted sender');
    expect(host.ensureConnection).not.toHaveBeenCalled();
    expect(request).not.toHaveBeenCalled();
  });

  it.each([
    [WORKSPACE_RUNTIME_IPC_CHANNELS.create, { name: '' }],
    [WORKSPACE_RUNTIME_IPC_CHANNELS.bindFolder, { workspaceId: 'workspace-1' }],
    [WORKSPACE_RUNTIME_IPC_CHANNELS.list, []],
    [WORKSPACE_RUNTIME_IPC_CHANNELS.update, { workspaceId: 'workspace-1' }],
    [WORKSPACE_RUNTIME_IPC_CHANNELS.delete, { workspaceId: '' }],
  ])('connects before rejecting invalid %s payloads without transport', async (channel, value) => {
    const { handlers, host, order, request } = fixture();
    await expect(handlers.get(channel)!('trusted', value)).rejects.toThrow(/Invalid/);
    expect(host.ensureConnection).toHaveBeenCalledOnce();
    expect(order).toEqual(['source', 'connect']);
    expect(request).not.toHaveBeenCalled();
  });

  it('preserves connection and transport failure identity', async () => {
    const connectionFixture = fixture();
    const offline = new Error('offline');
    connectionFixture.host.ensureConnection.mockRejectedValue(offline);
    await expect(
      connectionFixture.handlers.get(WORKSPACE_RUNTIME_IPC_CHANNELS.list)!('trusted', {}),
    ).rejects.toBe(offline);
    expect(connectionFixture.request).not.toHaveBeenCalled();

    const transportFixture = fixture();
    const failure = new Error('pipe failed');
    transportFixture.request.mockRejectedValue(failure);
    await expect(
      transportFixture.handlers.get(WORKSPACE_RUNTIME_IPC_CHANNELS.delete)!('trusted', {
        workspaceId: 'workspace-1',
      }),
    ).rejects.toBe(failure);
  });
});
